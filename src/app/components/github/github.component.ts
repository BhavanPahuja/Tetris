...
export class GithubComponent {
  ...
  tweetMaxScoreShareUrl = computed(() => {
    const text = encodeURIComponent(
      `Woo-hoo! I got a ${this.max()} points on Angular Tetris. Wanna join the party?`
    );
    return `https://twitter.com/intent/tweet?text=${text}&hashtags=${HASHTAG}`;
  });

  //eslint-disable-next-line max-len
  tweetAngularTetrisUrl = `https://twitter.com/intent/tweet?text=Awesome%20Tetris%20game%20built%20with%20Angular%20and%20Akita%2C%20can%20you%20get%20999999%20points%3F&hashtags=${HASHTAG}`;
